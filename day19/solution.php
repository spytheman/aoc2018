...
//printf("ipidx: %d\n",$ipidx); $programsize = count($program)-1; for($i=0;$i<=$programsize;$i++)printf("I: %-3d CMD: %s\n",$i, state2string($program[$i]));

function simulateProgram(array $program, int $ipidx, array $cpustate ): array {
    ...
    $c=0; while(true){
        ...
        if( $c > 30 ){
            // Heuristic speedup (determined by looking hard at goal and purpose of the input):
            $n = Amax($cpustate);
...
printf("Part 1 answer (content of register 0, after program termination): %s\n", simulateProgram($program, $ipidx, [0,0,0, 0,0,0])[0]);
printf("Part 2 answer (content of register 0, after program termination): %s\n", simulateProgram($program, $ipidx, [1,0,0, 0,0,0])[0]);

function state2string($state){ $res = []; foreach( $state as $rname=>$rval) $res[]=sprintf("%8s",$rval);  return '['.join(',', $res).']'; }
